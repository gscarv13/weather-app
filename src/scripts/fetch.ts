...
async function getWeatherNextDays(location: location) {
  const url: string = `https://api.openweathermap.org/data/2.5/onecall?lat=${location.lat}&lon=${location.lon}&units=metric&exclude=hourly,minutely&appid=ebe2305c30c57f985e0cf559531b323d`;
...
async function getWeather(location: string) {
  const url: string = `https://api.openweathermap.org/data/2.5/weather?q=${location}&units=metric&lang=${navigator.language}&appid=ebe2305c30c57f985e0cf559531b323d`;
  const res: Response = await fetch(url, { mode: 'cors' });
  const data: any = await res.json();
